...
export class SyncRecriaBatchUseCase {
    ...
    async execute(dto: SyncRecriaDto, idUser: number): Promise<SyncRecriaResponseDto> {
        ...
        return {
            ...
            totalFailed: weightRecords.failed + rearingSelections.failed,
            weightRecords,
            rearingSelections,
        };
    }
    ...
    private async processRearingSelections(
        ...
        for (const op of operations) {
            try {
                ...
                this.logError('rearing_selection', op.localId, op.operation, error);
                results.push({ localId: op.localId, status: 'failed', error: this.extractMessage(error) });
            }
        }
    ...
    private logError(entity: string, localId: string, operation: string, error: any): void {
        this.logger.warn(`Sync RECRÍA: localId="${localId}" entity="${entity}" op="${operation}" → ${error?.message ?? error}`);
    }
}
