...
import { RegisterVaccinationDto } from 'src/app/health/dto/inputs/register-vaccination.dto';
import { UpdateVaccinationDto } from 'src/app/health/dto/inputs/update-vaccination.dto';
import { RegisterTreatmentDto } from 'src/app/health/dto/inputs/register-treatment.dto';
import { UpdateTreatmentDto } from 'src/app/health/dto/inputs/update-treatment.dto';
import { RegisterHealthIncidentDto } from 'src/app/health/dto/inputs/register-health-incident.dto';
import { UpdateHealthIncidentDto } from 'src/app/health/dto/inputs/update-health-incident.dto';
...
export class SyncSanidadBatchUseCase {
    ...
    async execute(dto: SyncSanidadDto): Promise<SyncSanidadResponseDto> {
        ...
        const vaccinationsSection = await this.processVaccinations(dto.vaccinations ?? [], localIdToServerId);
        const treatmentsSection = await this.processTreatments(dto.treatments ?? [], localIdToServerId);
        const healthIncidentsSection = await this.processHealthIncidents(dto.healthIncidents ?? [], localIdToServerId);

        return {
            totalSucceeded: vaccinationsSection.succeeded + treatmentsSection.succeeded + healthIncidentsSection.succeeded,
            totalFailed: vaccinationsSection.failed + treatmentsSection.failed + healthIncidentsSection.failed,
            vaccinations: vaccinationsSection,
            treatments: treatmentsSection,
            healthIncidents: healthIncidentsSection,
        };
    }
