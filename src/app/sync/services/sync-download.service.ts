...
export class SyncDownloadService {
    ...
    async getRanchesForUser(idUser: number): Promise<SyncRanchDto[]> {
        const ranchUsers = await this.ranchUsersService.findByUser(idUser);
    ...
    async assertRanchAccess(idUser: number, idRanch: number): Promise<void> {
        ...
        if (!ranchUser) {
            throw new MyForbiddenException('No tiene acceso a esta estancia', 'RANCH_ACCESS_DENIED');
        }
    }

    async getCatalogs(): Promise<SyncCatalogsResponseDto> {
        const [animalClasses, animalBreeds, animalStatuses, eventTypes, productiveStatuses, productionTypes] =
            await Promise.all([
    ...
    private buildTableConfigs(): TableConfig[] {
        return [
            ...
            },
            {
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(AnimalEvent, 'ae', 'ae.id = bs.idEvent')
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = ae.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
            {
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(AnimalEvent, 'ae', 'ae.id = gd.idEvent')
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = ae.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
            {
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(AnimalEvent, 'ae', 'ae.id = p.idEvent')
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = ae.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
            {
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(AnimalEvent, 'ae', 'ae.id = w.idEvent')
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = ae.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
            {
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = h.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
            {
                key: 'weightRecords',
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(AnimalEvent, 'ae', 'ae.id = wr.idEvent')
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = ae.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
            {
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(AnimalEvent, 'ae', 'ae.id = rs.idEvent')
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = ae.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
            {
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(AnimalEvent, 'ae', 'ae.id = fe.idEvent')
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = ae.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
            {
            ...
            },
            {
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(Movement, 'mv', 'mv.id = ma.idMovement')
                        .where('mv.idRanch = :idRanch', { idRanch }),
            },
            {
                key: 'animalExits',
                table: 'animal_exits',
                ...
                applyRanchFilter: (qb, idRanch) =>
                    qb
                        .innerJoin(AnimalEvent, 'ae', 'ae.id = aex.idEvent')
                        .innerJoin(RanchAnimal, 'ra', 'ra.id = ae.idRanchAnimal')
                        .where('ra.idRanch = :idRanch', { idRanch }),
            },
        ];
