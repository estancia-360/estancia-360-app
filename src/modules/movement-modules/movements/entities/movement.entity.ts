...
import { MovementAnimal } from '../../movement-animals/entities/movement-animal.entity';
...
export class Movement extends BaseCreatedUpdated {
    ...
    @Column({ name: 'id_ranch', type: 'bigint', nullable: false })
    idRanch: number;

    @Column({ name: 'id_user', type: 'bigint', nullable: true })
    idUser?: number;

    @Column({ name: 'movement_type', type: 'varchar', length: 30, nullable: false })
    movementType: MovementTypeEnum;

    @Column({ name: 'movement_date', type: 'date', nullable: false })
    movementDate: Date;
