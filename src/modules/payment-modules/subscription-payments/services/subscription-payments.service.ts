...
export class SubscriptionPaymentsService {
    constructor(
        ...
        private readonly rawRepo: Repository<SubscriptionPayment>,
    ) {}
    ...
    async create(
        ...
        const repo = manager?.getRepository(SubscriptionPayment) ?? this.rawRepo;
    ...
    async findByRanchSubscription(idRanchSubscription: number, manager?: EntityManager): Promise<SubscriptionPaymentDto[]> {
        const repo = manager?.getRepository(SubscriptionPayment) ?? this.rawRepo;
        const payments = await repo.find({ where: { idRanchSubscription }, order: { paymentDate: 'DESC' } });
