...
import { RanchSubscriptionNotFoundException } from '../exceptions/ranch-subscription-not-found.exception';
import { SubscriptionCapacityExceededException } from '../exceptions/subscription-capacity-exceeded.exception';
...
export class RanchSubscriptionsService {
    constructor(
        ...
        private readonly ranchSubscriptionsRepository: Repository<RanchSubscription>,
        private readonly subscriptionPlansService: SubscriptionPlansService,
    ) {}

    async createFreeSubscription(idRanch: number, manager?: EntityManager): Promise<RanchSubscription> {
        const repo = manager?.getRepository(RanchSubscription) ?? this.ranchSubscriptionsRepository;
        ...
        const subscription = new RanchSubscription();
    ...
    async activatePlan(
        ...
        const em = manager ?? this.ranchSubscriptionsRepository.manager;
        ...
        if (Number(plan.priceMonthly) > 0 && !data.billingCycle) {
            throw new MyBadRequestException(
                `El plan "${plan.name}" requiere especificar billingCycle (monthly o annual).`,
        ...
        // columnas del UPDATE en vez de mandar NULL (mismo problema ya documentado
        // para RanchAnimal.idLot) — por eso se arma el UPDATE explícito acá.
    ...
    async extendPeriod(idRanch: number, months: number, manager?: EntityManager): Promise<RanchSubscription> {
        ...
        const now = new Date();
        const baseSource = subscription.currentPeriodEnd && new Date(subscription.currentPeriodEnd) > now
            ? new Date(subscription.currentPeriodEnd)
            : now;
        // TypeORM lee columnas DATE parseándolas como medianoche UTC, pero al
        // persistir usa getters LOCALES (getFullYear/getMonth/getDate) — hay que leer
        // los componentes en UTC y reconstruir con el constructor local, si no el día
        // persistido queda corrido según el timezone del server (ver activatePlan()).
        const base = new Date(
            baseSource.getUTCFullYear(),
            baseSource.getUTCMonth() + months,
            baseSource.getUTCDate(),
        );
    ...
    async getMetrics(manager?: EntityManager): Promise<{
        ...
        for (const subscription of subscriptions) {
            ...
            // nada, y el MRR alimenta decisiones reales (ver finanzas-flujo-caja.md,
            // activación de sueldos por umbral de MRR sostenido).
            const status = this.getEffectiveStatus(subscription);
            if (status !== 'active') continue;

            const plan = subscription.plan!;
            activeClients += 1;
            mrr += subscription.billingCycle === BillingCycleEnum.ANNUAL
