...
export class RanchAnimalsService {
    ...
    async create<T>(dto: CreateRanchAnimalDto, returnDto: new () => T): Promise<T> {
        const existingCode = await this.findOneByCode(RanchAnimalPlainDto, dto.code, { throwException: false });
        ...
        if (dto.codeMother) {
            animal.idMother = (await this.findOneByCode(RanchAnimalPlainDto, dto.codeMother))!.id;
        }
        if (dto.codeFather) {
            animal.idFather = (await this.findOneByCode(RanchAnimalPlainDto, dto.codeFather))!.id;
        }
    ...
    async update<T>(id: number, dto: UpdateRanchAnimalDto, returnDto: new () => T): Promise<T> {
        ...
        if (dto.idRanch) {
            animal.idRanch = (await this.ranchesService.findOneById(RanchDto, dto.idRanch)).id;
        }
        if (dto.idStatus) {
    ...
    findOneByCode<T>(dto: new () => T, code: string, options: { throwException: false }): Promise<T | null>;
    findOneByCode<T>(dto: new () => T, code: string, options?: FindOptions): Promise<T>;
    async findOneByCode<T>(dto: new () => T, code: string, { throwException = true }: FindOptions = {}): Promise<T | null> {
        const result = await this.repo.findOne({ dto, where: { code } });
