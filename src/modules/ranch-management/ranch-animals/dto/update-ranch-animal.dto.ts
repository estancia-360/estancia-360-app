...
import { IsInt, IsPositive, IsOptional, IsString, IsNotEmpty, MaxLength, IsDateString, IsNumber, Min, IsIn } from 'class-validator';

export class UpdateRanchAnimalDto {
    @ApiProperty({ description: 'ID de la estancia a la que pertenece el animal', example: 1, required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'Código único del animal', example: 'VAC-001', required: false })
    @IsOptional()
    @IsString({ message: 'El código debe ser un texto' })
    @IsNotEmpty({ message: 'El código no puede estar vacío si se envía' })
    @MaxLength(50, { message: 'El código no puede superar los 50 caracteres' })
    ...
    @ApiProperty({ description: 'Fecha de nacimiento del animal', example: '2022-05-10', required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'Sexo del animal (F = Hembra, M = Macho)', example: 'F', required: false })
    @IsOptional()
    @IsString({ message: 'El sexo debe ser un texto' })
    @IsIn(['F', 'M'], { message: 'El sexo solo puede ser F (Hembra) o M (Macho)' })
    sex?: 'F' | 'M';

    @ApiProperty({ description: 'Fecha y hora en la que el animal fue registrado en el dispositivo', example: '2026-01-24T10:30:00.000Z', required: false })
    @IsOptional()
